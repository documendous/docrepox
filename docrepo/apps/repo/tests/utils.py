...
def get_test_folder(name=None, parent=None, owner=None):
    test_folder = Folder.objects.create(
        name="Test Folder" if not name else name,
        owner=owner if owner else get_test_user(),
...
def get_test_document(name=None, parent=None, owner=None, ext="txt"):
