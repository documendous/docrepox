...
def remove_bookmark(user, element_type, element_pk):
    ...
    bookmark.delete()
    log.debug(f"Bookmark deleted for {element_pk}")


def add_bookmark(user, element_type, element_pk):
    log = logging.getLogger(__name__)
    ...
    if not Bookmark.objects.filter(
        ...
        Bookmark.objects.create(owner=user, content_object=element)
        log.debug(f"Created bookmark on {element} for {user}")
